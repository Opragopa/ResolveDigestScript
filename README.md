...
Генератор выпуска из шести новостей для одной Fusion-композиции в DaVinci Resolve.
...
```
title_01 … title_06     # Text+ с заголовками
body_01 … body_06       # Text+ с описаниями
image_01 … image_06     # Loader с картинками
```
...
Шесть повторяющихся блоков: заголовок, один или несколько абзацев описания,
...
Номер в `(фото 6)` — порядковый номер фото **в статье**, а не имя файла.
При недостаточном числе картинок запуск останавливается: неверное фото не будет
подставлено молча.
...
выберите DOCX и папку для кэша. Никаких аргументов в Terminal для ежедневной
работы не требуется.

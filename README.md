...
получает путь к этой папке. Если в DOCX указано `(фото N)`, `Trim In` и `Trim Out`
устанавливаются в `N-1–N-1`; без метки используется `0–0`. После запуска нужное
фото можно выбрать вручную в Inspector Loader.

...
`Z:/Workspace/<год> year/<месяц>_<название>/<день> <месяц>/`. После обновления
композиции автоматически запускается render preset `дайджест на экраны` с
этой же папкой назначения.

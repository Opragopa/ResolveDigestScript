...
скачивает все содержательные фотографии каждой статьи в отдельную
последовательность Loader. Если метки нет, `Trim In` и `Trim Out` по умолчанию
выбирают первое фото (`0–0`); для `(фото 6)` выбирается кадр `5–5`. После
запуска выбор можно вручную поменять в Inspector ноды `image_XX`, поскольку
остальные кадры последовательности уже сохранены. При недостаточном числе
картинок для указанного номера запуск останавливается.
